...
sys.setrecursionlimit(10**5)  # Aumenta o limite de recursão para evitar erros com vetores grandes

def ordered_vector(num):
    return list(range(1, num + 1))  # Gera um vetor ordenado
...
def plot_graph(vet_length, tempo_exe, title):
    ...
    plt.savefig("ordenado.png")  # Salva o gráfico em uma imagem
...
def main():
    ...
    for num in vet_length:
        vet = ordered_vector(num)  # Gera vetor ordenado
    ...
    plot_graph(vet_length, tempo_exe, "Comparação de Algoritmos de Ordenação (n log n)- Vetor Ordenado")
